...
public class UserGroupManagement 
{
	...
	@Test(priority=1)
	public void CreateUserGroup(String Groupname, ITestContext context) throws InterruptedException
	{
		//Select Security manager
		driver.findElement(By.xpath("//div[3]/app-menu-item-template/div/div/i")).click();
		Thread.sleep(4000);
		driver.findElement(By.xpath("//div[3]/app-menu-item-template/div/div[2]/app-menu-item-template[2]/div/div/span")).click();
		Thread.sleep(8000);         
		...
		try
		{
			...
			String UserGroupmanagemnetdata=driver.findElement(By.xpath("//app-simple-table/div/div")).getText();
			System.out.println("user group data is: " +UserGroupmanagemnetdata);
			
			if(UserGroupmanagemnetdata.contains(Groupname))
			{
				System.out.println("Execute user group delete");
				this.Groupdeleting();
			}
			else
			{
		...
		//give the user name
		driver.findElement(By.id("userGroupName")).clear();
		driver.findElement(By.id("userGroupName")).sendKeys(Groupname);
		Thread.sleep(2000);
		
		//Give principle
		driver.findElement(By.id("principleName")).clear();
		driver.findElement(By.id("principleName")).sendKeys(".*");
		Thread.sleep(2000);
		...
		if(Usergroupmanagemnetdata.contains(Groupname))
		{
			System.out.println("User group is created");
			context.setAttribute("Status",1);
			context.setAttribute("Comment", "User group is created successfully");
		}
		else
		{
			System.out.println("User group is not created");
			context.setAttribute("Status",5);
			context.setAttribute("Comment", "User group is created successfully");
			driver.findElement(By.id("Create user group failed")).click();
		}
	}
	
	@TestRail(testCaseId=1187)
	@Parameters({"Groupname", "Description"})
	@Test(priority=2, dependsOnMethods = {"CreateUserGroup"})
	public void EditUserGroup(String Groupname, String Description, ITestContext context) throws InterruptedException
	{
		...
		//get the Description
		String ResultDes=driver.findElement(By.xpath("//td[6]")).getText();
		System.out.println("Result description data is: " +ResultDes);
		
		if(ResultDes.equalsIgnoreCase(Description))
		{
			System.out.println("User group is updated successfully");
			context.setAttribute("Status",1);
			context.setAttribute("Comment", "User group is updated successfully");
		}
		else
		{
			System.out.println("User group is not updated");
			context.setAttribute("Status",5);
	...
	@TestRail(testCaseId=1188)
	@Parameters({"Groupname"})
	@Test(priority=3, dependsOnMethods = {"CreateUserGroup"})
	public void DeleteUserGroup(String Groupname, ITestContext context) throws InterruptedException
	{
		...
		if(Resultusers.contains(Groupname))
		{
			System.out.println("User group is not deleted");
			context.setAttribute("Status",5);
			context.setAttribute("Comment", "User group is not deleted");
			driver.findElement(By.id("User group delete failed")).click();
		}
		else
		{
			System.out.println("User group is deleted");
			context.setAttribute("Status",1);
			context.setAttribute("Comment", "User group is deleted successfully");
		}
	}
	
	
	public void Groupdeleting() throws InterruptedException
	{		
		//click on checkbox and choose edit option
		driver.findElement(By.xpath("//td/input")).click();
		Thread.sleep(3000);
